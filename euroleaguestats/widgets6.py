...
from library2 import check_opponents
from library4 import per_game_statistics, per_game_stats, fetch_opponents
from widgets5 import RV


def access_per_game_stats(tree, name):

    """Fetches stats for all games in each phase."""

    k = per_game_stats(tree)

    '''Creates dict with all opponents for each phase.'''
    y = check_opponents(tree)
    q = fetch_opponents(tree, y[0], y[1], y[2])

    '''Creates 3 dicts, each, filled with the opponents of each phase, plus the 'round' number.
    '''
