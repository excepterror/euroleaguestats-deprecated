...
from participants import get_opponents_dict_on_press
...
def access_bio(roster_n, player_name):
    ...
    player_photo_link = v[0].xpath('//div[@class="player-img"]/img/@src')

    if not player_photo_link:
        player_photo = 'Images/NoImage.jpg'

    else:
        response = requests.get(player_photo_link[0])
        player_photo = v[1] + '.jpg'

        with open(player_photo, 'wb') as f:
            f.write(response.content)

    bio = player_info + player_pos + [player_photo]
    name = v[1]
    tree = v[0]
