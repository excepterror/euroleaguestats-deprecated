...
def get_opponents(tree, rivals_rs, rivals_po, rivals_ff):
    ...
    for i, rival in enumerate(rivals_ff):
        finalfour_game = int(round_num_ff_n[i]) - int(max(round_num_po_n))
        opponents_dict_po.update(
            {"FinalFour - Game " + str(finalfour_game) + ":" + " " + rival: int(round_num_ff_n[i])})
